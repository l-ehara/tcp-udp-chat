...
def handle_sendtxt(sender_nickname, recipient_nickname, file_contents, sender_address):
    if recipient_nickname in nicknames:
        recipient_index = nicknames.index(recipient_nickname)
        recipient_client = clients[recipient_index]
        server.sendto(f'File from {sender_nickname}: {file_contents}'.encode('ascii'), recipient_client)
    else:
        server.sendto(f'{recipient_nickname} is not online.'.encode('ascii'), sender_address)
    ...
    while True:
        try:
            ...
            if message.startswith('/pm'):
                ...
                handle_pm(sender_nickname, recipient_nickname, private_message, client_address)
